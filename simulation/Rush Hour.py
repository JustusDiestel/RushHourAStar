...
class game:  # main class
    ...
    def clickObject(self):  # when the window is clicked
        for x in range(len(self.rectObjects)):  # for every object
            if self.rectObjects[x].rect.collidepoint(
                    self.ev.pos):  # if the coordinates of the click is within a rectangle
                self.rectObjects[x].rectDrag = True  # make it be in the air
                self.mouseX, self.mouseY = self.ev.pos  # get current mouse position
                self.offsetX = self.rectObjects[
                                   x].rect.x - self.mouseX  # get different between mouse and rectangle coordinates
        ...
        for x in range(len(self.rectObjects)):  # for each rectangle
            if self.rectObjects[x].rectDrag:  # if the rectangle is in the air
                ...
                if self.rectObjects[x].orientation == "h":
                    countStart = int(self.rectObjects[
                                         x].currentX / 80)  # get the starting square that is needed to be checked for collisions
                ...
                for y in range(len(temporaryRectangles)):  # for each rectangle
                    for z in range(countStart, countEnd + 1):  # for each square between the move
                        if self.rectObjects[x].orientation == "h":
                            middleX = ((z * perSq) + (((z + 1) * perSq) + (
                            ((self.rectObjects[x].size - 1) * perSq)))) / divisor  # get the new middle coordinate
                            middleX2 = (((z * perSq) + (
                                        ((z + 1) * perSq) + (((self.rectObjects[x].size - 1) * perSq)))) / divisor) * (
                                                   divisor - 1)  # for size 3 cars
                            ...
                            if ((temporaryRectangles[y].startX <= middleX <= (
                                    temporaryRectangles[y].extendX + temporaryRectangles[y].startX)) or (
                                        temporaryRectangles[y].startX <= middleX2 <= (
                                        temporaryRectangles[y].extendX + temporaryRectangles[y].startX))) and (
                                    temporaryRectangles[y].startY <= middleY <= (
                                    temporaryRectangles[y].extendY + temporaryRectangles[y].startY)):
                                moveAllowed = False
                ...
                if (self.rectObjects[x].startLimitX <= jumpX < self.rectObjects[x].endLimitX) and (
                        self.rectObjects[x].startLimitY <= jumpY < self.rectObjects[x].endLimitY) and moveAllowed:
                    # update the necessary attributes of the rectangle
                    self.rectObjects[x].rect = pygame.Rect(jumpX, jumpY, self.rectObjects[x].extendX,
                                                           self.rectObjects[x].extendY)
                    ...
                    self.rectObjects[x].rect = pygame.Rect(self.rectObjects[x].currentX, self.rectObjects[x].currentY,
                                                           self.rectObjects[x].extendX, self.rectObjects[x].extendY)
                    self.rectObjects[x].rectDrag = False
    ...
    def loadGame(self):  # reading the file
        ...
        for line in lines:
            if line.strip():  # ignoriert leere Zeilen
                self.carInfos.append(line.strip().split(', '))
